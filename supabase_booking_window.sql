...
--
-- KURAL: Kullanıcılar yalnızca Cumartesi 09:00'dan sonra yeni haftanın randevularını oluşturabilir.
...
BEGIN
  ...
  -- Eğer randevu gelecek haftaya aitse ve henüz Cumartesi 09:00 olmadıysa engelle
  IF appt_week_monday >= next_week_monday THEN
    -- Cumartesi 09:00 sonrasına kadar gelecek hafta randevusu oluşturulamaz
    IF NOT (current_dow = 6 AND current_hour >= 9) AND current_dow != 0 THEN
      RAISE EXCEPTION 'Randevular henüz açılmadı! Cumartesi saat 09:00''dan sonra randevu alabilirsiniz.';
    END IF;
  END IF;
